...
class VideoFrameReconstructor_LessDownSample(nn.Module):
    def __init__(self, C, num_frames, LSTM_hidden_size, num_layers, encoder_final_channel = 512):
        super(VideoFrameReconstructor_LessDownSample, self).__init__()
...
class Encoder(nn.Module):
    def __init__(self, input_channels, final_channel, bilinear=False):
        ...
        self.encode_double_conv2 = DoubleConv(128, 256)
        factor = 2 if bilinear else 1
        self.encode_double_conv3 = DoubleConv(256, final_channel // factor)


    def forward(self, x):
        x1 = self.inc(x)
        x2 = self.down1(x1)
        x3 = self.encode_double_conv2(x2)
        x4 = self.encode_double_conv3(x3)
...
class Decoder(nn.Module):
    def __init__(self, input_channels = 256, final_channel = 3, bilinear=False):
        ...
        self.decode_double_conv2 = DoubleConv(128 // factor, 64)
        self.decode_double_conv3 = DoubleConv(64 // factor, 32)

        self.outc = OutConv(32, final_channel)

    def forward(self, x):
        x = self.up1(x)
        x = self.decode_double_conv2(x)
        x = self.decode_double_conv3(x)
        logits = self.outc(x)
        return logits
