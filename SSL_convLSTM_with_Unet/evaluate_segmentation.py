...
def evaluate(model, reconstructor, dataloader, device, amp):
    model.eval()  # Ensure the network is in evaluation mode
    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
            ...
            masks_pred = model(reconstructed_frames)
            ...
            jaccard_score = jaccard(masks_pred_softmax, F.one_hot(true_masks, num_classes=model.n_classes).permute(0, 3, 1, 2).float())
