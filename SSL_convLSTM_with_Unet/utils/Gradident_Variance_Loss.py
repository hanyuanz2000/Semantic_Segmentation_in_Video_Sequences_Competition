...
class GradientVariance(nn.Module):
    ...
    - device: torch.device, the device on which to run calculations (e.g., 'cpu', 'cuda', 'mps').
    """
    def __init__(self, patch_size, device):
        ...
        self.kernel_x = torch.FloatTensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]).unsqueeze(0).unsqueeze(0).to(device)
        self.kernel_y = torch.FloatTensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]).unsqueeze(0).unsqueeze(0).to(device)
